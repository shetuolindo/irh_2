...
@section('page_script')
  <script>
    $("#search_box").keypress(function(event){
        ...
        if(keycode == '13'){
            window.location = "{{ route('admin.view.savedResource.list') }}" + "?search=" + $("#search_box").val();
        }
    });

    $("#refine_btn").click(function(){
      window.location = "{{ route('admin.view.savedResource.list') }}" + "?res=" + $("#filter_drop1").val() + "&age=" + $("#filter_drop2").val() + "&sort=" + $("#filter_drop3").val();
    });
  </script>
@endsection