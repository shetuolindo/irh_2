...
@section('page_title', 'Viewing History')
@section('content')
                ...
                @endif
                  <div class="myResourceProSub">
                   <table class="table" id="resources-table">
                      ...
                      <tbody>
                         @forelse($viewingHistory as $sViewingHistory)
                            <tr data-row-id="{{ $sViewingHistory->id }}">
                               <td><a href="{{ route('theme.singleresource',$sViewingHistory->resource_id) }}" target="_blank"><img src="{{ !blank($sViewingHistory->preview_attachment) ? asset('irh_assets/uploads/resource_preview/' . $sViewingHistory->preview_attachment) : asset('irh_assets/images/resource_thum.jpg') }}" width="142" height="88" alt=""/></a></td>
                               <td>{{ date('d/m/Y', strtotime($sViewingHistory->created_at)) }}</td>
...
@section('page_script')
  <script>
    $(".del-link").click(function(){
       var eThis = $(this);
       $.ajax({
          url: ("{{ url('/admin/viewing-history/del/') }}" + "/" + $(this).closest('tr').attr('data-row-id')),
          method: "GET",
    ...
    $("#del_btn").click(function(){
      ...
      var eThis = $(this);
       $.ajax({
          url: ("{{ url('/admin/viewing-history/batch-del/') }}" + "/" + selval),
          method: "GET",
          success:function(msg){
