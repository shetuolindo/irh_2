...
@section('page_title', 'Published Resource')
@section('content')
      ...
      <div class="container">
         <div class="row adminProfile">
            <div class="myResourcePro">
               <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 float-left" id="pubdiv">
                  @include('dashboard.admin.resources.published_resource_pagination')
...
@section('page_script')
   <script>
      $(document).on("click", ".del-link", function(){
         var eThis = $(this);
         $.ajax({
            url: ("{{ url('/admin/published-resource/del') }}" + "/" + $(this).closest('tr').attr('data-row-id')),
            method: "GET",
      ...
      function ban(data, id){
         $.ajax({
            url: ("{{ url('/admin/published-resource/ban') }}" + "/" + id),
            method: "POST",
            data:data,
            success:function(msg){
               if(msg.prev == '1'){
      ...
      $(document).on("click", ".featured-link", function(){
         var eThis = $(this);
         $.ajax({
            url: ("{{ url('/admin/published-resource/featured') }}" + "/" + $(this).closest('tr').attr('data-row-id')),
            method: "GET",
            success:function(msg){
               if(msg.prev == '1'){
      ...
      $("#search_box").keyup(function(event){
            $.ajax({
             url: "{{ (route('admin.publishedResources.list.pajax') . '?query=') }}" + $("#search_box").val(),
             method:"GET",
