         ...
         <div class="row adminProfile">
            <div class="myResourcePro">
               <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 float-left" id="flagdiv">
                  @include('dashboard.admin.resources.flag_resource_pagination')
...
@section('page_script')
   <script>
      ...
      $(document).on("click", ".admin-notice", function(){
         var eThis = $(this);
         $.ajax({
            url: ("{{ url('/admin/flaged-resource/notice') }}" + "/" + $(this).closest('tr').attr('data-row-id')),
            method: "GET",
            success:function(msg){
               /*if(msg.prev == '1'){
      ...
      $(document).on('click', '.remove-flag', function(){
         var eThis = $(this);
         $.ajax({
            url: ("{{ url('/admin/flaged-resource/rem-flag/') }}" + "/" + $(this).closest('tr').attr('data-row-id')),
            method: "GET",
            success:function(msg){
               /*if(msg.prev == '1'){
                  eThis.find('.fa').removeClass('fa-star').addClass('fa-star-o');
      ...
      $("#search_box").keyup(function(event){
            $.ajax({
             url: "{{ (route('admin.flagedResources.list.pajax') . '?query=') }}" + $("#search_box").val(),
             method:"GET",
