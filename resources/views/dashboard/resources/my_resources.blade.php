         ...
         <div class="row adminProfile">
            <div class="myResourcePro">
               <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 float-left" id="myres">
                  @include('dashboard.resources.my_resources_pagination')
               </div>
            </div>
         </div>
      </div>
...
@section('page_script')
   <script>
      ...
      $(document).on("click", ".del-link", function(){
         var eThis = $(this);
         $.ajax({
            url: ("{{ url('/user/my-resources/del') }}" + "/" + $(this).closest('tr').attr('data-row-id')),
            method: "GET",
      ...
      $("#search_box").keyup(function(event){
            $.ajax({
             url: "{{ (route('user.myResources.list.pajax') . '?query=') }}" + $("#search_box").val(),
             method:"GET",
