         ...
         <div class="row adminProfile">
            <div class="myResourcePro">
               <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 float-left" id="pendiv">
                  @include('dashboard.resources.pending_resource_pagination')
               </div>
            </div>
         </div>
      </div>
...
@section('page_script')
   <script>
      $(document).on("click",".approve-link", function(){
         var eThis = $(this);
         $.ajax({
            url: ("{{ url('/user/pending-resource/app') }}" + "/" + $(this).closest('tr').attr('data-row-id')),
            method: "GET",
      ...
      $("#search_box").keyup(function(event){
            $.ajax({
             url: "{{ (route('user.pendingResources.list.pajax') . '?query=') }}" + $("#search_box").val(),
             method:"GET",
