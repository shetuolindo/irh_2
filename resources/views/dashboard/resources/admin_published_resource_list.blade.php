         ...
         <div class="row adminProfile">
            <div class="myResourcePro">
               <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 float-left" id="pubdiv">
                  @include('dashboard.resources.published_resource_pagination')
...
@section('page_script')
   <script>
      ...
      $(document).on("click", ".featured-link", function(){
         var eThis = $(this);
         $.ajax({
            url: ("{{ url('/user/published-resource/featured') }}" + "/" + $(this).closest('tr').attr('data-row-id')),
            method: "GET",
            success:function(msg){
               if(msg.prev == '1'){
      ...
      $("#search_box").keyup(function(event){
            $.ajax({
             url: "{{ (route('user.publishedResources.list.pajax') . '?query=') }}" + $("#search_box").val(),
             method:"GET",
